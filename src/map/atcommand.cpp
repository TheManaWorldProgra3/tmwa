...
static
ATCE atcommand_hair_stylew(Session *s, dumb_ptr<map_session_data> sd,
        ZString message)
{
    int hair_stylew = 0;

    if (!extract(message, &hair_stylew))
        return ATCE::USAGE;

    if (hair_stylew >= MIN_HAIR_STYLE && hair_stylew <= MAX_HAIR_STYLE)
    {
        {
            pc_changelook(sd, LOOK::HAIR, hair_stylew);
            clif_displaymessage(s, "Appearence changed."_s);
        }
    }
    else
